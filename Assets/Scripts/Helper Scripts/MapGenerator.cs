...
public class MapGenerator : MonoBehaviour {
	...
	//Vị trí mà tại đó các prefab tươn ứng đc render ra(setup trong inspector)
	...
	void InitializePlatform(GameObject prefab, ref Vector3 last_Pos, Vector3 last_Pos_Of_Tile,
		...
		for (int i = 0; i < amountTile; i++) {
			...
			//Tại đó là các prefab ở top gần road
			...
			//Tại đó là các prefab ở bottom gần road
			...
			//Tại đó là các prefab bên dưới cấp 2 (dưới đường , dưới cây)
			else if (clone.tag == MyTags.BOTTOM_FAR_LAND_2) {

				//Tại vị trí thứ 5 thì render ra bigtree
				if (orderInLayer == 5) {
					CreateTreeOrGround (big_Tree_Prefab, ref clone, new Vector3(-0.57f, -1.34f, 0f));
				}

			}
			//Bên trên cấp 2 , else này thực hiện khi và chỉ khi gọi với ground prefab 3
	...
	void CreateTreeOrGround(GameObject prefab, ref GameObject tileClone, Vector3 localPos) {
		...
		if(tileClone.tag==MyTags.BOTTOM_NEAR_GRASS){
			cloneRenderer.sortingLayerName = MyTags.SORTING_LAYER_TREE_BOTTOM;	
		}

		//Nếu các prefab cần render ra có vật thể trên bề mặt thì prefab3 (chỉ toàn đất) phải tắt đi để show child của nó
	...
	void SetNearScene(GameObject bigGrassPrefab, ref GameObject clone, ref int orderInLayer,
		...
		//Tại mỗi thứ tự vị trí đã set trong inspector thì tại đó render ra prefab ta mong muốn
		...
		//Tương ứng với mỗi thứ tự vị trí của cây
